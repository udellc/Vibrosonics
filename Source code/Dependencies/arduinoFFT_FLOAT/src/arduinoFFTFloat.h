...
public:
  /* Constructor */
  arduinoFFT(void);
  arduinoFFT(float *vReal, float *vImag, uint16_t samples,
  ...
  void MajorPeak(float *vD, uint16_t samples, float samplingFrequency,
                 float *f, float *v);
  void Windowing(float *vData, uint16_t samples, FFTWindow windowType,
                 FFTDirection dir);
...
private:
  ...
  void Parabola(float x1, float y1, float x2, float y2, float x3,
