...
arduinoFFT::arduinoFFT(float *vReal, float *vImag, uint16_t samples,
...
void arduinoFFT::Compute(float *vReal, float *vImag, uint16_t samples,
                         FFTDirection dir) {
#warning("This method is deprecated and may be removed on future revisions.")
...
// Compute the FFT
  ...
  if (dir != FFT_FORWARD) {
    float reciprocal = 1.0 / this->_samples;
...
void arduinoFFT::ComplexToMagnitude(float *vReal, float *vImag,
                                    uint16_t samples) {
// vM is half the size of vReal and vImag
#warning("This method is deprecated and may be removed on future revisions.")
  for (uint16_t i = 0; i < samples; i++) {
...
void arduinoFFT::Windowing(float *vData, uint16_t samples,
...
float arduinoFFT::MajorPeak(float *vD, uint16_t samples,
...
void arduinoFFT::MajorPeak(float *vD, uint16_t samples,
                           float samplingFrequency, float *f, float *v) {
#warning("This method is deprecated and may be removed on future revisions.")
  float maxY = 0;
  uint16_t IndexOfMaxY = 0;
  // If sampling_frequency = 2 * max_frequency in signal,
...
void arduinoFFT::Parabola(float x1, float y1, float x2, float y2, float x3,
