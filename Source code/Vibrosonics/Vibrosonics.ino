...
#define DEFAULT_NUM_WAVES 7             // The number of waves to synthesize, and how many slices the breadslicer function does
...
#define FREQ_MAX_AMP_DELTA_MIN 100      // The threshold for a change in amplitude to be considered significant by the frequencyMaxAmplitudeDelta() function

#define FREQ_MAX_AMP_DELTA_K 0.2        // The K-value used to weight amplitudes that are not the amplitude of most change
...
const int breadslicerSliceLocationsStatic[DEFAULT_NUM_WAVES] {250, 500, 900, 1600, 3000, 4500, 5000};  // array storing pre-defined slice locations in array for slicing the FFT amplitudes array (vReal)
int breadslicerSliceLocations[DEFAULT_NUM_WAVES];                               // array for storing values caluclated for slicing the FFT amplitudes array

const float breadslicerSliceWeights[DEFAULT_NUM_WAVES] {1.0, 0.8, 1.0, 2.0, 2.5, 3.0, 5.0}; // weights associated to the average amplitudes of slices
...
void updateControl() {
  ...
  else if (nextProcess == FADE_RATE - 1) {
  ...
  for (int i = 0; i < numWaves; i++) {
    ...
    if (gainValue > 0) {
      ...
      if (frequency < BASS_FREQ) {
        frequency = int(round(frequency * 0.26));
      // shrink the rest of the peak frequencies by mapping them to them to a range
        ...
        normalizedFreq = pow(normalizedFreq, 0.9);
        frequency = int(round(normalizedFreq * 100) + 20);
      }
