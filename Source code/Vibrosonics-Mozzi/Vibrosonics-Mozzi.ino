...
########################################################
  ...
  To ensure this, each updateControl() call is dedicated to a certain process, which are divided into four phases. 
    ...
    4. Phase D - calls the remaining FFT functions FFTCompute() and ComplexToMagnitude(). This phase is also dedicated to all signal processing such
        ...
        NOTE: to use data from averaged FFT windows, pass "FFTData" as parameter to breadslicer, otherwise pass "vRealR"
    OTHER:
...
// Audio input pins
#define AUDIO_INPUT_PIN_RIGHT A2       // left channel audio input
#define AUDIO_INPUT_PIN_LEFT A3      // right channel audio input
...
float vRealR[FFT_WIN_SIZE];  // vRealR is used for input from the left channel and receives computed results from FFT
float vImagL[FFT_WIN_SIZE];  // vImagL is used to store imaginary values for computation

float vRealRPrev[FFT_WINDOW_SIZE_BY2]; // stores the previous amplitudes generated by FFT
...
arduinoFFT FFT = arduinoFFT(vRealR, vImagL, FFT_WIN_SIZE, SAMPLING_FREQ); // Object for performing FFT's
...
void updateControl() {
  ...
  // FFT Phase A, functions
    ...
    // store previously computed FFT results in vRealRPrev array, and copy values from audioInputBuffer to vReal array
    setupFFT();
  ...
  // FFT phase C, and signal processing phase
    ...
    averageFFTWindows();
    // Use the breadslicer function to process FFT output, pass "FFTData" as a parameter if using averaged FFT windows, otherwise pass "vRealR"
    breadslicer(vRealR);
...
void recordSample() {
  ...
  for (int i = 0; i < numSamplesPerProcess; i++) {
    sampleT = micros();
    audioInputBuffer[numSamplesTaken] = mozziAnalogRead(AUDIO_INPUT_PIN_RIGHT);
...
void setupFFT() {
  for (int i = 0; i < FFT_WINDOW_SIZE; i++) {
    // store the previously generated amplitudes in the vRealRPrev array, to smoothen the spectrogram later during processing
    if (i < FFT_WINDOW_SIZE_BY2) {
      vRealRPrev[i] = vRealR[i];
    }
    // copy values from audio input buffers
    vRealR[i] = float(audioInputBuffer[i]);
...
void averageFFTWindows () {
  for (int i = 0; i < FFT_WINDOW_SIZE_BY2; i ++) {
    FFTData[i] = (vRealRPrev[i] + vRealR[i]) / 2.0;
  }
}
  ...
  for (int i = 0; i < numWaves; i++) {
    ...
    if (gainValue > 0) {
      ...
      // shrink the rest of the peak frequencies by mapping them to them to a range
        ...
        frequency = int(round(float(normalizedFreq * float(SYNTH_MAX_MIN_DIFF)) + SYNTH_MIN_FREQ));
      }
