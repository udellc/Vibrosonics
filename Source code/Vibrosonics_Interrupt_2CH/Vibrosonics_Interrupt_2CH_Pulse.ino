...
int pulse_generator_state = 0;

// used for detecting percussion
float inst_noisiness = 0;
float prev_noisiness = 0;
float inst_mean_amp = 0;
float prev_mean_amp = 0;
...
void loop() {
  if (AUD_IN_BUFFER_FULL()) {
    ...
    if(inst_noisiness > 0.80 && 
    inst_mean_amp > (10 * prev_mean_amp))
    {  
      Serial.println("PULSE\n");
      pulse_generator_state = 3;
    }
    generatePulse();
...
void processData() {
  ...
  calculate_noisiness(freqs);
  //Serial.printf("inst_noisiness: %f\n", inst_noisiness);
  //Serial.printf("inst_mean_amp: %f\n", inst_mean_amp);
  //Serial.printf("prev_mean_amp: %f\n", prev_mean_amp);
...
float calculate_noisiness(float *data) {

  // save historical data
  prev_noisiness = inst_noisiness;
  prev_mean_amp = inst_mean_amp;

  int num_bins = FFT_WINDOW_SIZE_BY2 - 1;

  // find max bin amplitude and calculate average amplitdue per bin
  float amp_sum = 0;
  float max_amp = 0;
  for(int i=0; i<num_bins; i++){
    amp_sum += data[i];
    if(max_amp < data[i]) max_amp = data[i];
  }
  float mean_amp = amp_sum / num_bins;
  inst_mean_amp = mean_amp;

  // calculate noisiness
  // a mean amplitude less than 1% of the max amplitude indicates a lack of meaninful data to analyze
  float noisiness = 0;
  if(mean_amp > (FFT_MAX_AMP / 100.0)){
    noisiness = 1 - ((max_amp / mean_amp) / 66.7);
  }
  inst_noisiness = noisiness;
...
void generateAudioForWindow() {
  for (int i = 0; i < AUD_OUT_BUFFER_SIZE; i++) {
    ...
    //for (int c = 0; c < 1; c++) {
      ...
      generateAudioBuffer[0][generateAudioIdx] += get_sum_of_channel(0) * cos_wave_w[i];
      generateAudioBuffer[1][generateAudioIdx] = 0.0;
    //}
...
void generatePulse(){
  if(pulse_generator_state == 0) return;

  for (int i = 0; i < AUD_OUT_BUFFER_SIZE; i++) {
    float sin_value = get_sin_wave_val(5 + (10 * pulse_generator_state-1), 128, 0);
    generateAudioBuffer[1][generateAudioIdx] += sin_value * cos_wave_w[i];
  
    // copy final, synthesized values to volatile audio output buffer
    if (i < AUD_IN_BUFFER_SIZE) {
      // shifting output by 128.0 for ESP32 DAC, min max ensures the value stays between 0 - 255 to ensure clipping won't occur
      AUD_OUT_BUFFER[1][generateAudioOutIdx] = max(0, min(255, int(round(generateAudioBuffer[1][generateAudioIdx] + 128.0))));
      generateAudioOutIdx += 1;
      if (generateAudioOutIdx == AUD_OUT_BUFFER_SIZE) generateAudioOutIdx = 0;
    }
    // increment generate audio index
